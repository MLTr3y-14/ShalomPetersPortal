...
var wp_bogus_attributes = /^(_wpro_data|_base_href|_moz_dirty|_moz_editor_bogus_node|_done)$/i;
...
function wp_e(name) {
	return wp_getEditorByName(name);
}
...
function wp_gethtml(node,obj) {
	...
	var n = scriptTags.length
	for (var i = 0; i < n; i++) {
		scriptTags[i]._wpro_data = scriptTags[i].text;
	}
	
	var cn = node.childNodes
	var n = cn.length
	for (var i = 0; i < n; i++) {
		wp_appendNodeHTML(cn[i], sb, obj, false)
	}
	// 222
...
function wp_escapeCharacters (str, range, mapping) {
	if (!mapping) {
		mapping = [];
	}
	var arr = [];
...
// end 222
function wp_fixAttribute(value, obj) {
	value = String(value).replace(/\&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\"/g, "&quot;").replace(/\xA0/g, "&nbsp;");
	eval('value.replace(/'+String.fromCharCode(160)+'/g, \'&nbsp\')');
	if (obj.escapeCharacters) {
		value = wp_escapeCharacters(value);
	}
	return value;
}
function wp_fixText(text, inPre, obj) {
	if (!inPre) {
		text = String(text).replace(/\n/g, ' ');
	}
	value = String(text).replace(/\&/g, "&amp;").replace(/</g, "&lt;").replace(/([^-][^-])>/g, "$1&gt;").replace(/\"/g, "&quot;").replace(/\xA0/g, "&nbsp;");
	eval('value.replace(/'+String.fromCharCode(160)+'/g, \'&nbsp\')');
	if (obj.escapeCharacters) {
		value = wp_escapeCharacters(value);
	}
...
function wp_getAttributeValue(attrNode, elementNode, sb, obj) {
	...
	if (name != "style") {
		if (!isNaN(value)) {
			...
			//}
			if (value == null) return;
			value = wp_urlFormatting(value, obj);
		}
		sb.append(" " + name + "=\"" + wp_fixAttribute(value, obj) + "\"")
	} 
}
wp_urlFormatting = function (url, obj) {
	// skip js links
	if (url.match(/^javascript:/gi)) {
		return url;
	}	
	// prevent anchor links to the editor
	var locationRegex = new RegExp('^'+wp_quoteMeta(obj._location.replace(/&/gi,'(&|&amp;)'))+'#', 'gi');
	url = url.replace(locationRegex, '#');
	var location = obj._location.replace(/^([\s\S]*\/)[^\/]*$/i, "$1");
	if (obj.fullURLs) {
		if (obj._baseDomain) {
		...
		if (obj._baseDomain) {
			var loc = obj._baseDomain;
		} else {
			var loc = obj._domain;
		}
		var domainRegex = new RegExp('^'+wp_quoteMeta(loc) +'[^\/]*(|/)', 'gi');
		url = url.replace(domainRegex, '$1');
	}
...
function wp_rgbToHex(R,G,B) {return wp_toHex(R)+wp_toHex(G)+wp_toHex(B)}
function wp_toHex(N) {
 if (N==null) return "00";
...
function wp_hexToR(h) {return parseInt((wp_cutHex(h)).substring(0,2),16)}
function wp_hexToG(h) {return parseInt((wp_cutHex(h)).substring(2,4),16)}
function wp_hexToB(h) {return parseInt((wp_cutHex(h)).substring(4,6),16)}
function wp_cutHex(h) {return (h.charAt(0)=="#") ? h.substring(1,7):h}
function wp_hexToRGB(h) {return 'rgb('+wp_hexToR(h)+', '+wp_hexToG(h)+', '+wp_hexToB(h)+')'}

function wp_styleFormatting (str, raw, obj) {
		
	if (/style="/.test(str)) {
		str = str.substring(7);
	}
	if (raw) {
		str = wp_htmlSpecialCharsDecode(str);
	}
	// build styles array
	var arr = {};
		...
		for (var i=0; i<n; i++) {
			...
			if (s[0] && s[1]) {
				...
				if (obj.border_visible == 1) {
					if (/1px dashed rgb\(127, 124, 117\)/i.test(s[1])) continue;
					if (/#7f7c75 1px dashed/i.test(s[1])) continue;
				}			
				arr[s[0].toLowerCase()] = s[1].replace(/^([^\s]+)/, ' $1');
			}
		}
		
		wp_compressBoxStyles(arr, 'border', '-width', 'border-width');
		wp_compressBoxStyles(arr, 'border', '-color', 'border-color');
		wp_compressBoxStyles(arr, 'border', '-style', 'border-style');
		wp_compressBoxStyles(arr, 'border', '', 'border');
		wp_compressBoxStyles(arr, 'padding', '', 'padding');
		wp_compressBoxStyles(arr, 'margin', '', 'margin');
		
		wp_shorthandStyles(arr, 'list-style', ['-type','-position','-color']);
		wp_shorthandStyles(arr, 'border', ['-color','-width','-style']);
		wp_shorthandStyles(arr, 'border-top', ['-color','-width','-style']);
		wp_shorthandStyles(arr, 'border-right', ['-color','-width','-style']);
		wp_shorthandStyles(arr, 'border-bottom', ['-color','-width','-style']);
		wp_shorthandStyles(arr, 'border-left', ['-color','-width','-style']);
		wp_shorthandStyles(arr, 'background', ['-color','-image','-repeat','-attachment','-position']);
...
wp_urlEncode = function (sStr) {
	return escape(sStr).replace(/\+/g, '%2C').replace(/\"/g,'%22').replace(/\'/g, '%27');
}
wp_urlDecode = function (psEncodeString) {	
 //alert (unescape(psEncodeString))
 return unescape(psEncodeString); 
// return psEncodeString.replace(/%20/g, ' ')
}
wp_addSlashes = function (str) {
	return str.replace(/\\/gi, "\\\\").replace(/'/gi, "\\'").replace(/"/gi, '\\"');
}
wp_htmlSpecialChars = function (str) {
	return str.replace(/&/gi, '&amp;').replace(/\xA0/gi, '&nbsp;').replace(/</gi, '&lt;').replace(/>/gi, '&gt;').replace(/"/gi, '&quot;')	
}
wp_htmlSpecialCharsDecode =  function (str) {
	return str.replace(/&nbsp;/gi, String.fromCharCode(160)).replace(/&lt;/gi, '<').replace(/&gt;/gi, '>').replace(/&quot;/gi, '"').replace(/&amp;/gi, '&')	
}

function wp_appendNodeHTML(oldnode, sb, obj, inPre) {
	
	var node = oldnode.cloneNode(true);
	if (node.nodeType==1) {
		if (oldnode.getAttribute('width')) {
			node.setAttribute('width', oldnode.getAttribute('width'));
		}
		if (oldnode.getAttribute('height')) {
			node.setAttribute('height', oldnode.getAttribute('height'));
		}
	}
	...
	switch (node.nodeType) {
		case 1:	// ELEMENT		
			...
			if (!name || name == '') {
				...
				l = cs.length
				for (var i = 0; i < l; i++) {
					wp_appendNodeHTML(cs[i], sb, obj, inPre)
				}
				break
			}
			...
			if (node.style.cssText.length > 1) {
				sb.append(' style="')
				sb.append(wp_htmlSpecialChars(wp_styleFormatting(node.style.cssText, false, obj)));
				sb.append('"')
			}
			// attributes
			var attrs = node.attributes
			...
			if ((node.canHaveChildren || node.hasChildNodes() || wp_cant_have_children_has_close_tag.test(name) ) && name != 'basefont' && name != 'base' && name != 'script' && name != 'style' && name != 'title') {
				...
				// end 222
					...
					// Mozilla noscript handling fix
					if (name=='noscript' && !wp_is_ie) {
						sb.append(node.innerHTML.replace(/&lt;/, '<').replace(/&gt;/, '>'));
					} else 
				...
				sb.append(">" + (node._wpro_data?node._wpro_data:node.text) + "</" + name + ">")
			} else if (name == "style") {
				...
				if (node.innerHTML) {
					...
					sb.append(">" + obj.edit_object.document.title + "</" + name + ">")  
				}
		...
		case 3:	// TEXT
			if (node.nodeValue) {
				...
				sb.append( wp_fixText( str, inPre, obj ))
			}
			break
		case 4:
			sb.append("<![CDA" + "TA[\n" + node.nodeValue + "\n]" + "]>")
...
function wp_show_menu(obj, type, srcElement) {	
	...
	if (frame.style.display=="none") {
		...
		if (writeIt) {
			var frameDoc = eval("obj."+type+"_frame.document")
			var menu_code
			if (obj.styles == '') {
...
function wp_findit(obj) {
	...
	var findwin = obj.openDialog(wp_directory + "find.php?lang="+obj.instance_lang, 'modal', 318, 146)
}
	...
	for (var i=0; i < l; i++) {
		if (tables[i].border == 0 || tables[i].border == null) {
			if (wp_is_ie) {
				...
				tables[i].style.border = "1px dashed #7F7C75";
			}
