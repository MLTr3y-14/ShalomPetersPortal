...
function wp_editor(obj,config) {
	...
	// objects:
	obj.html_edit_area = wp_getEditorByName(obj.id); //document.getElementById(obj.id)
	...
	if (obj.form) obj.form.addEventListener('submit', eval('obj.'+obj.id+'_submit' ), false)
	// end	
...
function wp_load_data(name) {
	var obj = wp_getEditorByName(name);
...
function wp_send_to_html(obj) {
	...
	str = str.replace(/ _moz_resizing="true"/gi, '');
...
function wp_send_to_edit_object(obj, init) {
	...
	// wp 222
	eval("obj.edit_object.document.addEventListener('mouseup', wp_"+obj.id+"_mouseUpHandler, true)")
	eval("obj.edit_object.document.addEventListener('mousedown', wp_closePopup, true)")
	eval("obj.edit_object.document.addEventListener('keypress', wp_"+obj.id+"_keyHandler, true)")
	eval("obj.edit_object.document.addEventListener('contextmenu', wp_"+obj.id+"_contextHandler, true)")
	if (init) {
...
function wp_callFormatting(obj,sFormatString) {
	...
	var container = startContainer.parentNode
	// find the parent node	
	var p = wp_skipInline(container)
	var t = p.tagName;
	if (t == 'BODY' || (t=='DIV' && obj.usep)) {
		obj.edit_object.document.execCommand('formatblock', false, ((obj.usep) ? 'P' : 'DIV') )
	}
...
function wp_open_hyperlink_window(obj,srcElement) {
	...
	if (data) {
		...
		var szURL=wp_directory + "link.php?target="+escape(thisTarget)+"&title="+escape(thisTitle) + "&lang="+obj.instance_lang
...
function wp_open_image_window(obj) {
	...
	if (imageNode) {
		if ((imageNode.getAttribute('name')) && (imageNode.src.search(wp_directory+"images/bookmark_symbol.gif") != -1)) {
			...
			if (!iborder) iborder = '';
			szURL= wp_directory + 'imageoptions.php' + '?thumbnails='+obj.thumbnails +'&width=' + width +'&height=' + height + '&alt=' + escape(alt) + '&align=' + align + '&mtop=' + mtop + '&mbottom=' + mbottom + '&mleft=' + mleft + '&mright=' + mright + '&border=' + iborder + "&lang="+obj.instance_lang +'&image=' + escape(image) 
		}
	} else {
		szURL= wp_directory + obj.imagewindow + "?thumbnails="+obj.thumbnails+"&instance_img_dir="+obj.instance_img_dir+"&lang="+obj.instance_lang 
	}
...
function wp_create_hr(obj,align, color, size, width,percent2) {
	...
	if (parentTag.tagName == 'P') {
		...
		parentTag.parentNode.insertBefore(hr, parentTag);
	} else {
