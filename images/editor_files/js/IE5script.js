...
function wp_editor(obj,config) {
	// Strings:
	obj.thumbnails = config.thumbnails;
	...
	obj.fullURLs = config.fullURLs
	obj.escapeCharacters = config.escapeCharacters
	...
	obj.html_edit_area = wp_getEditorByName(obj.id); //eval("document.getElementById('"+obj.id+"')")
	...
	// submit_form
	eval('obj.'+obj.id+'_submit = function (evt) { wp_prepare_submission('+obj.id+'); }' );
	if (obj.form) obj.form.attachEvent('onsubmit', eval('obj.'+obj.id+'_submit' ))
	// end

	/* create the regex strings for removing the domain etc */	
	obj._location = document.location.toString();
	obj._domain = obj._location.replace(/(^(http|https):\/\/[^\/]*)[\s\S]*/i, '$1');
	if (obj.baseURLurl) {
		var loc = obj.baseURLurl;
...
function wp_next (obj) {
	...
	// end 222
	try {
		obj.edit_object.document.open('text/html', 'replace')
	} catch (e) {
	...
	// 2212
	/*if (wp_is_ie7) {
	obj.edit_object.document.body.contentEditable = true;
	}*/
	// end 2212
	...
	if (document.attachEvent) {
		eval("obj.edit_object.document.attachEvent('onmouseup', wp_"+obj.id+"_mouseUpHandler, true)")
		eval("obj.edit_object.document.attachEvent('onmousedown', wp_"+obj.id+"_mouseDownHandler, true)")
	} else {
		eval("obj.edit_object.document.onmouseup = wp_"+obj.id+"_mouseUpHandler")
	}
...
function wp_mouseDownHandler(obj, evt) {
	if (wp_is_ie7) {
		if (!obj.initfocus) {
			obj.edit_object.document.body.contentEditable = true;
		}
...
function wp_open_hyperlink_window(obj,srcElement) {
	...
	if (data) {
		...
		var szURL= wp_directory +  "dialog_frame.php?target="+escape(thisTarget)+"&title="+escape(thisTitle)+"&lang="+obj.instance_lang+"&window="+wp_directory+"link.php"
...
function wp_hyperlink(obj,iHref,iTarget,iTitle) {
	...
	if (iHref=="") { 
		...
		if (this_href.tagName == "A") {
			...
			var reg_ex = /^mailto:([^?]*)($|[\s\S]*$)/i;
			if (reg_ex.test(this_href.innerHTML)) {
				this_href.innerHTML = this_href.innerHTML.replace(reg_ex,'$1');
			}
		}
	}
	obj.edit_object.focus()
	...
	if (obj.edit_object.document.selection.type == "Control") {
		...
		if (sel(0).tagName == "IMG") {
			if ((sel(0).getAttribute('name')) && (sel(0).src.search(wp_directory+"images/bookmark_symbol.gif") != -1)) {
				...
				szURL= wp_directory + 'dialog_frame.php' + '?thumbnails='+obj.thumbnails +'&width=' + width +'&height=' + height + '&alt=' + escape(alt) + '&align=' + align + '&mtop=' + mtop + '&mbottom=' + mbottom + '&mleft=' + mleft + '&mright=' + mright + '&border=' + iborder + "&lang="+obj.instance_lang +"&window="+wp_directory+"imageoptions.php" + '&image=' + escape(image)
			}
		} else {
			return
		}
	} else {
		szURL= wp_directory + 'dialog_frame.php?window=' + wp_directory + obj.imagewindow + '&thumbnails='+obj.thumbnails+"&instance_img_dir="+obj.instance_img_dir+"&lang="+obj.instance_lang 
	}
...
function wp_open_bookmark_window(obj,srcElement) {	
	...
	bookwin = obj.openDialog(wp_directory + "bookmark.php?bookmark="+escape(arr)+"&lang="+obj.instance_lang, 'modal', 300, 106)
}
