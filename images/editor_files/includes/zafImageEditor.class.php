<?php
//if (!defined('IN_WP')) exit;

class zafImageEditor extends wpImageEditor {
...
class wpImageEditor {
		...
		if (empty($output)) {
			...
			if (@imagejpeg($rotated, $output, $quality)) { //image destination
				...
				if (!empty($this->fileCHMOD)) {
					...
					wp_chmod($output, $this->fileCHMOD);
				}
			}
		}
	...
	*/
	...
	function _imageResizeJpeg( $file, $output, $origwidth, $origheight, $width, $height, $cropX=0, $cropY=0, $quality=60 ) {
		...
		if (empty($output)) {
			header("Content-type: image/jpeg", true);
			if (@imagejpeg($base_image)) {
				$return = array($width, $height, $output);
			}
		} else {
			$fh=@fopen($output,'w');
			@fclose($fh);
			if (@imagejpeg($base_image, $output)) { //image destination
				$return = array($width, $height, $output);
				if (!empty($this->fileCHMOD)) {
					//$fs = new wpFilesystem();
					//$fs->chmod($output, $this->fileCHMOD);
