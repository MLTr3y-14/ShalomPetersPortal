...
class wysiwygPro {
	...
	var $thumbnails = 'true';
	var $hyperlink_function = 'open_hyperlink_window(wp_e(\'##name##\'),this)';
	...
	var $format_menu = '
		<div class="off" onClick="parent.wp_change_format(parent.wp_current_obj,\'&lt;##lineReturns##&gt;\');off(this)" onMouseOver="on(this)" onMouseOut="off(this)"><nobr> 
			<##lineReturns## style="position: static; float: none; clear: both; display: block; visibility: visible">##normal##</##lineReturns##>
			</nobr></div>
	...
	*/
	...
	function wysiwygPro($save_name='') {
		...
		$opera_version = preg_replace('/.*opera\//smi', '', $browser_string);
		...
		/*} elseif($is_opera && $opera_version>=9) {
			$this->is_gecko = true;
			$add_to_cache_name = '_gecko';
			$add_to_cache_name .= '14';
			$this->gecko_revision = 1.5;*/
		
		/*} elseif ($is_safari && $safari_version >= 412) {
			$this->is_gecko = true;
			$add_to_cache_name = '_gecko';
			$add_to_cache_name .= '14';
			$this->gecko_revision = 1.5;
	...
	*/
	function wp_entities($code) {
		...
		if ($this->unsupported) return $code;
	...
	*/
	...
	function set_doctype($doctype='<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">') {
		if ($this->has_expired) {
		...
		if ($this->has_expired) {
			if (!empty($baseURL)) {
				if (substr($baseURL, strlen($baseURL) - 1) != '/' ) {
					if (preg_match('/^http(|s):\/\/[^\/]*?$/smi', $baseURL )) {
						$baseURL.='/';
					} else {
						$baseURL = preg_replace('/^(.*\/)[^\/]*?$/smi', "$1", $baseURL);
					}
				}
		...
		if ($this->has_expired) {
			if (!empty($stylesheet)) {
				if (is_array($stylesheet)) {
					//array_merge($this->stylesheets, $stylesheet);
					//$this->set_style = implode("','", $stylesheet);
					foreach ($stylesheet as $k) {
						array_push($this->stylesheets, $k);
					}
				} else {
	...
	*/
	...
	/***************************************************************************
	 disablethumbnails
	 Public: disables thumbnails in the image manager.
	...
	*/
	...
	function disablelinkmngr($disable=true) {
		if ($this->has_expired) {
			if ($disable) {
				$this->hyperlink_function = "callFormatting(wp_e(\'##name##\'),'CreateLink')";
			...
			if (!is_array($links)) {
				...
				for ($i=0; $i<$num_links; $i++) { 
					if ((!empty($links[$i][1])) && (!empty($links[$i][2]))) {
						$links[$i][1] = addslashes(str_replace(array("\t","\r\n","\r","\n",'"','<','>'), array(' ',' ',' ',' ','&quot;','&lt;','&gt;'), $links[$i][1]));
						$links[$i][2] = addslashes(str_replace(array("\t","\r\n","\r","\n",'"','<','>'), array(' ',' ',' ',' ','&quot;','&lt;','&gt;'), $links[$i][2]));
						array_push($arr, '['.intval($links[$i][0]).',"'.$links[$i][1].'","'.$links[$i][2].'"]');
					}	
				}
...
<script language="JavaScript" type="text/javascript" src="##directory##js/loader.js?version=##version##"></script>
...
';
			...
			// document stylesheet
			array_push($this->stylesheets, WP_WEB_DIRECTORY.'document_theme.css');			
			...
			$this->WYSIWYGPRO_code = $this->wp_replace($this->WYSIWYGPRO_code, array(  
				...
				'stylesheet' => empty($this->stylesheets) ? "''" : '[\''.implode("','", $this->stylesheets).'\']',
				...
				'fullURLs' => $this->fullURLs?'true':'false',
				'escapeCharacters' => $this->escapeCharacters?'true':'false',
...
// return error reporting level to original state
if (isset($WP_PRE_ERROR_LEVEL)) {
	error_reporting($WP_PRE_ERROR_LEVEL);
}
?>