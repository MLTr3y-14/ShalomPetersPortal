			...
			if(isset($_POST['1stTerm']))
			{
				...
				if ($num_rows3 > 0 ) {
					while ($row3 = mysql_fetch_array($result3)) 
					{
						...
						$query4 = "select Payable,PaidAmount from tbfeepayment where ReceiptNo IN (Select ID From tbreceipt where AdmnNo = '$Stu_AdmnNo' And Term = '1st Term' And Status = '0' and Session_ID = '$Session_ID' and Term_ID = '$Term_ID') And ChargeID = '$chargeID' and Session_ID = '$Session_ID' and Term_ID = '$Term_ID'";
			...
			if(isset($_POST['2ndTerm']))
			{
				...
				if ($num_rows3 > 0 ) {
					while ($row3 = mysql_fetch_array($result3)) 
					{
						...
						$query4 = "select Payable,PaidAmount from tbfeepayment where ReceiptNo IN (Select ID From tbreceipt where AdmnNo = '$Stu_AdmnNo' And Term = '2nd Term' And Status = '0' and Session_ID = '$Session_ID' and Term_ID = '$Term_ID') And ChargeID = '$chargeID' and Session_ID = '$Session_ID' and Term_ID = '$Term_ID'";
						$result4 = mysql_query($query4,$conn);
						$dbarray4 = mysql_fetch_array($result4);
						$AmtPayable  = $dbarray4['Payable'];
						$PaidAmount  = $dbarray4['PaidAmount'];
			...
			if(isset($_POST['3rdTerm']))
			{
				...
				if ($num_rows3 > 0 ) {
					while ($row3 = mysql_fetch_array($result3)) 
					{
						...
						$query4 = "select Payable,PaidAmount from tbfeepayment where ReceiptNo IN (Select ID From tbreceipt where AdmnNo = '$Stu_AdmnNo' And Term = '3rd Term' And Status = '0' and Session_ID = '$Session_ID' and Term_ID = '$Term_ID') And ChargeID = '$chargeID' and Session_ID = '$Session_ID' and Term_ID = '$Term_ID'";
						$result4 = mysql_query($query4,$conn);
						$dbarray4 = mysql_fetch_array($result4);
						$AmtPayable  = $dbarray4['Payable'];
						$PaidAmount  = $dbarray4['PaidAmount'];
...
function setstudentfee(){
	...
	alert('View/Update Fee Payment for ' + studentname);
	//alert('im good');
	dojo.xhrGet({
         ...
         error: helloError,
         content: {name: dijit.byId('selectclass').attr("value"), name2: studentname }
      });
	 }
...
function helloError1(data, ioArgs) {
	...
	function AddStudentPayment() {
		...
		for ( var i = 1; i <= chargelength; i++ ){
	        ...
	        var StudentBalance = parseInt(StudentCharge) - parseInt(StudentPayment);
					...
					<TR>
					  <TD colspan="3" valign="top"  align="left">Please don't leave any blank space (paid amount should be in figures only)
						  ...
						  <tr bgcolor="#ECE9D8">
							...
							<td width="118"><strong>Particulars</strong></td>
							<td width="117"><div align="center"><strong>Amount</strong></div></td>
							<td width="121"><div align="center"><strong>Paid Amount</strong></div></td>
							<td width="139"><div align="center"><strong>Balance</strong></div></td>
						  </tr>
                             <tr><td colspan="5" align="center"><table id="feedisplay" align="center"></table></td></tr>
							  

						   <tr>
							<td width="30">&nbsp;</td>
							<td width="118"><div align="right"><strong>TOTAL</strong></div></td>
						  ...
						  <tr>
							<td width="30">&nbsp;</td>
							<td width="118"><strong><div align="right">FINE</div></strong></td>
							<td width="117"><input name="FinedPayable" id="FinedPayable" type="text" size="13" onChange="AddStudentPayment();" style="text-align:right;"/></td>
						  ...
						  <tr>
                            ...
                            <td width="30">&nbsp;</td>
							<td width="118">&nbsp;</td>
							<td width="117"><div align="right"><strong>GRAND TOTAL</strong></div></td>
...
<?PHP
						$Count_Charge = 0;
						$query = "select ID,ChargeName from tbchargemaster where ChargeApplicable = '1' and Session_ID = '$Session_ID' and Term_ID = '$Term_ID' order by ChargeName";
						$result = mysql_query($query,$conn);
						$num_rows = mysql_num_rows($result);
						if ($num_rows <= 0 ) {
						...
						else 
						{
							while ($row = mysql_fetch_array($result)) 
							{
								...
								$query2 = "select * from tbmonthlyCharges where ChargeID='$chargeID' And Term = '$Activeterm' and Session_ID = '$Session_ID' and Term_ID = '$Term_ID'";
								$result2 = mysql_query($query2,$conn);
								$dbarray2 = mysql_fetch_array($result2);
...
<?PHP
						if(isset($_POST['GetStudentFee']))
						{
							...
							$query = "select Stu_Full_Name,AdmissionNo from tbstudentmaster where ID = '0' and Session_ID = '$Session_ID' and Term_ID = '$Term_ID' and Session_ID = '$Session_ID' and Term_ID = '$Term_ID' order by Stu_Full_Name";
						}
						$result = mysql_query($query,$conn);
						$num_rows = mysql_num_rows($result);
						if ($num_rows > 0 ) {
